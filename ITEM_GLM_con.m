...
% p-value p and stats, information about the test statistic.
% 
...
Finter = spm('FigName','ITEM_GLM_con: estimate');
spm_progress_bar('Init', 100, msg, '');

% Prepare inference
%-------------------------------------------------------------------------%
P    = inv(V);                  % precision matrix (inverse of covariance)
covB = inv(X'*P*X);             % covariance matrix (of the beta estimates)
